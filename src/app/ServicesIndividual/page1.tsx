  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      await SanityClient.fetch(
        `*[_type=='servicespage' && banner[0].title==${JSON.stringify(search)}]{
    thirdtitle,
    banner[]{
      title,
      content,
  ...
  return loader == true ? (
    ...
    <div className="overflow-hidden ">
      <div className="lg:h-[95vh]  h-[600px] relative w-full">
        <img
          className="absolute lg:flex hidden bottom-0"
          src="./sideline.png"
        ></img>
        <img
          className="absolute lg:flex hidden left-1/2 top-5"
          src="./circ.png"
        ></img>
        <img
          className="absolute lg:flex hidden left-1/4 bottom-10"
          src="./circ.png"
        ></img>
        <div className="lg:w-[70%]  overflow-hidden w-full  h-full lg:rounded-full shadow-primarycolor bg-gray-200 lg:drop-shadow-2xl absolute lg:top-5 lg:-right-[20%]">
          <img
            src={alldata?.banner[0]?.Image?.asset?.url}
        ...
        <div className="lg:text-6xl text-2xl xs:text-3xl w-full xl:text-7xl absolute   font-semibold lg:w-[40%]  lg:leading-[70px] text-wrap lg:top-1/2 lg:left-10   top-1/2 left-1/2 lg:-translate-x-0 lg:-translate-y-1/2 -translate-x-1/2  -translate-y-1/2 lg:text-start text-center ">
          <p className="sha bg-clip-text  w-full text-wrap mb-5">
          ...
          <div className="flex flex-col lg:justify-start lg:items-start justify-center items-center gap-4 ">
            ...
            <Link href={"/contactus"}>
              <button className="lg:bg-transparent bg-white border-2 border-white lg:border-black w-fit hover:bg-primarycolor duration-200 hover:text-white rounded-lg text-sm px-4 py-3">
                EXPLORE
              </button>
            </Link>
          </div>
      ...
      <div className="py-10">
        {alldata &&
          alldata.Secondsection &&
          alldata.Secondsection.map((item, index) =>
            index % 2 != 0 ? (
              <div
                key={index}
                className="flex justify-between  gap-5 items-center md:flex-row-reverse md:p-8 p-5 flex-col w-full "
              >
              ...
              <div
                key={index}
                className="flex justify-between  items-center md:flex-row flex-col gap-5 w-full md:p-8 p-5"
              >
                <div className="flex flex-col gap-5 md:text-start text-center md:w-1/2">
                  <h1 className="text-3xl font-semibold uppercase">
                    {alldata && item?.title}
      ...
      <div className={`w-full pb-5 ${alldata?.thirdsection === null ? "hidden" : ""}`}>
        <div className="w-full flex flex-col gap-5 ">
          <h1 className="text-center md:text-3xl sm:text-2xl text-xl font-semibold">
            {alldata && alldata?.thirdtitle}
          </h1>
          <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 md:gap-10 p-8 justify-items-center">
            {alldata?.thirdsection?.map((item: any, index: any) => (
              <div
                key={index}
                className="flex justify-center items-center p-3 shadow-2xl drop-shadow-2xl flex-col gap-5 xs:w-72 w-full h-80 rounded-lg border-2 border-primarycolor"
              >
                <img
                  src={item?.Image?.asset?.url}
                  alt=""
