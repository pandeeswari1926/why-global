...
gif:[{gif:{asset:{url:string}},percentage:string,content:string}],
...
WorkProcess:[{title:string,subTitle:string,card:[{gif:{asset:{url:string}},cardTitle:string,content:string}]}],
teamMember:[{title:string,subTitle:string,card:[{image:{asset:{url:string}},content:string,name:string,role:string}]}],
ourCulture:[
  {
    title:string,
...
asset->{
          ...
          <div>
            <div className="lg:absolute relative lg:w-[70%] w-full md:top-[65%] lg:top-[43%] xl:top-[35%]  md:left-0 pl-10 z-10">
              <div className="grid lg:grid-cols-4 grid-cols-2 p-2 justify-evenly">
                {item&&item.gif&&item.gif.map((list:any,listindex:any)=>(
  ...
  {list?.content}
      ...
      <div className="w-full flex flex-col justify-center items-center mt-7 ">
        <div className="relative w-full">
          <div className="flex lg:flex-row flex-col w-full  justify-center items-center gap-6 ">
            {alldata&&alldata.SecondSection.map((item:any,index:any)=>(
   ...
   <span className="text-3xl text-gray-400 font-semibold">{item.number}</span>
   <span className="text-sm lg:w-[80%] text-gray-400">{item.content}</span>
 </p>
</div>
            ))}
      ...
      <div className="w-full h-fit relative bg-no-repeat ">
        ...
        {alldata&&alldata.WhyGlobl.map((item:any,index:any)=>(
           <div key={index} className="flex lg:flex-row flex-col items-center justify-around">
        ...
        md:p-10"
           >
             <img src={item?.image?.asset?.url} className="w-full h-full" alt="" />
           </div>
           <div className="lg:w-[50%] w-full lg:text-left text-justify p-5 ">
             ...
             <br></br>
             {item&&item.content.map((list:any,index:any)=>(
...
{list?.content}
      ...
      {alldata&&alldata.WorkProcess&&alldata.WorkProcess.map((item:any,index:any)=>(
...
{item?.subTitle}
 ...
 <div className="grid md:grid-cols-3 md:p-10 p-5  grid-cols-1 lg:gap-16 gap-5 h-full w-full items-center  justify-items-center lg:px-20 ">
  {item&&item.card&&item.card.map((item:any,index:any)=>(
    <div className="lg:w-[90%] h-full relative drop-shadow-2xl">
     <div className={`border-2 ${index%2!=0?'lg:mt-10':'mt-0'} text-center bg-white rounded-tr-3xl`}>
       <img src={item?.gif?.asset?.url} alt="project" className="w-[80%]" />
...
<Video />
{alldata&&alldata.ourCulture&&alldata.ourCulture.map((item:any,index:any)=>(
...
<div className="w-full flex lg:flex-row flex-col gap-2">
  ...
  {item&&item.Fourimage.map((list:any,listItems:any)=>(
  <div className=" w-full relative">
      
  <img alt="" src={list?.image?.asset?.url} className="w-full h-full" />
  <div className="bg-black absolute top-0 left-0 w-full h-full opacity-50"></div> 
  <p className="flex flex-col absolute w-full bottom-2 left-1/2 -translate-x-1/2 text-white px-2"><span className="text-center text-primarycolor md:text-lg sm:text-xl text-lg font-semibold">{list?.heading}</span><span className="text-center text-sm lg:w-[80%] w-full mx-auto">{list?.content}</span></p>
  </div>
  ))}
...
<div className="bg-black absolute top-0 left-0 w-full h-full opacity-50"></div> 
  ...
  <p className="flex flex-col absolute w-full bottom-2 left-1/2 -translate-x-1/2 text-white px-2"><span className="text-center text-primarycolor md:text-lg sm:text-xl text-lg font-semibold">Ethical Responsibility</span><span className="text-center text-sm lg:w-[90%] w-full mx-auto">WHY Global Services thrives on fearless innovation. Our culture embraces creativity,encourages out-of-the-box thinking.</span></p>
      ...
      {alldata&&alldata.teamMember&&alldata.teamMember.map((item:any,index:any)=>(
      <div className="p-10">
        
        <p className="text-primarycolor font-semibold text-lg">{item?.title}</p>
        <h1 className="text-xl font-semibold text-gray-500">{item?.subTitle}</h1>
        <Slider {...settings} className="overflow-hidden">
          {item&&item.card.map((profile:any,index:any)=>(
             <div>
               ...
               <div className="w-full lg:pt-2">
                 ...
                 <div className="flex md:flex-row flex-col justify-between">
                   <div>
                     ...
                     <p className=" text-center text-[#FF8900]">{profile.role}</p>
                   </div>
