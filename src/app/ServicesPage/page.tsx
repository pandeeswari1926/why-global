...
function Page({ params }:any) {
 ...
 useEffect(()=>{
 const getdata=async()=>{
  await SanityClient.fetch(`*[_type=="servicespage"]{
    thirdtitle,
    banner[]{
      title,
      content,
 ...
 getdata()
    ...
    return ((loader==true?<Loader />:<div className="overflow-hidden ">
    ...
    <div className="flex justify-between  items-center md:flex-row flex-col w-full md:p-8 p-5">
      <div className="flex flex-col gap-5 md:text-start text-center md:w-1/2">
        <h1 className="text-3xl font-semibold">{alldata?.Secondsection[0]?.title}</h1>
